...
#define DLT_SWAP_64(value) \
  (((static_cast<uint64_t>(DLT_SWAP_32((value) & 0xffffffffull))) << 32u) | (DLT_SWAP_32((value) >> 32u)))

#define DLT_SWAP_16(value) ((((value) >> 8u) & 0xffu) | (((value) << 8u) & 0xff00u))
#define DLT_SWAP_32(value)                                                                  \
  ((((value) >> 24u) & 0xffu) | (((value) << 8u) & 0xff0000u) | (((value) >> 8u) & 0xff00u) \
...
#define DLT_STANDARD_HEADER_EXTRA_SIZE(htyp)                                                   \
  ((DLT_IS_HTYP_WEID(htyp) ? DLT_SIZE_WEID : 0) + (DLT_IS_HTYP_WSID(htyp) ? DLT_SIZE_WSID : 0) \
...
#define PRINT_FUNCTION_VERBOSE(_verbose) \
  if (_verbose) dlt_vlog(LOG_INFO, "%s()\n", __func__)
...
//  */
// typedef enum { DLT_RECEIVE_SOCKET, DLT_RECEIVE_UDP_SOCKET, DLT_RECEIVE_FD } DltReceiverType;
//
// /**
...
//   uint8_t headerbuffer[sizeof(DltStorageHeader) + sizeof(DltStandardHeader) + sizeof(DltStandardHeaderExtra)
//                        + sizeof(DltExtendedHeader)]; /**< buffer for loading complete header */
...
//   const char *ctid,
//   const int log_level,
//   const int32_t payload_min,
//   const int32_t payload_max,
//   int verbose);
// /**
 ...
 */
DltReturnValue ADAPTER_EXPORT dlt_check_storageheader(DltStorageHeader *storageheader);
...
//  * @param text pointer to a ASCII string, in which the output is written
//  * @param textlength maximal size of text buffer
//  * @param byteLength If argument is a string, and this value is 0 or greater, this value will be taken as string
//  length
//  * @param verbose if set to true verbose information is printed out.
//  * @return negative value if there was an error
//  */
...
//  */
// DltReturnValue dlt_log_init_multiple_logfiles_support(DltLoggingMode mode,
//   bool enable_multiple_logfiles,
//   int logging_file_size,
